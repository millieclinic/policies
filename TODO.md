...
- **DMARC / email authentication is not addressed in any policy.** Surfaced by the triple-check matrix as the only `None`-rated row across all 182 ECH questions (CSV row 55 — ISD IT Risk Assessment, Additional Cybersecurity §7). Add an email-authentication standards bullet under [network-and-cloud-security.md](New%20Policy%20Docs/network-and-cloud-security.md) §1 (or a new §1.x), and verify operational DMARC/SPF/DKIM posture on the millieclinic.com domain. Owner: Security Officer + CTO.
